...
fun main() {
    ...
    val grid: List<List<Char>> = {}.javaClass.getResourceAsStream("/day12/input.txt")?.bufferedReader()?.readLines()
        ?.mapIndexed { index, line ->
            if (line.contains("E")) {
                startX = index
                startY = line.indexOf("E")
            }
            line.replace("E", "z").toCharArray().toList()
        }.orEmpty()

    println("The answer is: ${bfs(grid, startX, startY)}")
}

fun bfs(grid: List<List<Char>>, startX: Int, startY: Int): Int {
    ...
    while (queue.isNotEmpty()) {
        ...
        if (grid[currentVertex.x][currentVertex.y] == 'a') {
